...
export class Vault {
    ...
    public async init(loginCallback: (vault: Vault) => Promise<any>) {
        ...
        try {
            ...
            throw new Error(
                "Vault.init(): Could not get health status of the vault. " +
                    "Please check the connecrtion. Error: " +
                    error,
            );
        }
    }
    ...
    public async userPassLogin(username: string, password: string, requiredPolicies: string[]) {
        const loginResp = await this.call(
            "POST",
    ...
    public async appRoleLogin(roleName: string, requiredPolicies: string[], roleId: string, roleSecret: string) {
        ...
        if (loginResp.data.auth.metadata.role_name !== roleName) {
            throw new Error(
                "This AppRole is not " + roleName + ", instead logged in as " + loginResp.data.auth.metadata.role_name,
            );
        }
    ...
    public async getStatus(): Promise<any> {
        ...
        try {
            ...
            if (
                ...
                ![200, 429, 472, 473, 501, 503].indexOf(error.response.status)
            ) {
                throw error;
            }
        }
    ...
    public async setSecret(secretPath: string, secret: any) {
        ...
        if (!resp || !resp.status || resp.status !== 204) {
            throw new Error("Error while setting the secret. Status is not 204");
        }
    }
    ...
    public async deleteSecret(secretPath: string) {
        ...
        if (!resp || !resp.status || resp.status !== 204) {
            throw new Error("Error while deleting the secret. Status is not 204");
        }
    }

    public async initVault(opts: {
        secret_shares: number;
        secret_threshold: number;
    }): Promise<{ root_token: string; keys: string[]; keys_base64: string[] }> {
        const resp: AxiosResponse<any> = await this.call(
            "POST",
            "/v1/sys/init",
            { secret_shares: opts.secret_shares, secret_threshold: opts.secret_threshold },
            undefined,
            false,
        );
    ...
    public async unseal(key: string) {
        await this.call("POST", "/v1/sys/unseal", { key }, undefined, false);
    }
    ...
    public async enableAuthMethod(method: string, description: string, config: {}) {
        await this.call("POST", "/v1/sys/auth/" + method, { type: method, description, config });
    }

    public async createUserpassUser(username: string, password: string, policies: string[]) {
        await this.call("POST", "/v1/auth/userpass/users/" + username, { password, policies: policies.join(",") });
    }

    public async createAppRoleRole(roleName: string, policies: string[]) {
        await this.call("POST", "/v1/auth/approle/role/" + roleName, { policies });
    }
    ...
    public async generateAppRoleSecretId(roleName: string, metadata: any): Promise<string> {
        const resp = await this.call("POST", "/v1/auth/approle/role/" + roleName + "/secret-id", {
    ...
    public async revokeToken(token: string) {
        await this.call("POST", "/v1/auth/token/revoke", { token });
    }
            ...
            (async () => {
                try {
                    ...
                    if (leaseDurationS > 240) {
                        ...
                        Log.log().debug(
                            "Vault token renewed. Valid for " +
                                leaseDurationS +
                                " seconds (until: " +
                                new Date(Date.now() + leaseDurationS * 1000).toISOString() +
                                " Next renew after " +
                                nextRenewAfterMs +
                                "ms",
                        );
                        ...
                        Log.log().debug(
                            "Vault token renewed. Valid for " +
                                ...
                                " seconds (until: " +
                                new Date(Date.now() + leaseDurationS * 1000).toISOString() +
                                " Lease duration is below the threshold. Performing login.",
                        );
                        await this.loginCallback(this);
                    }
                } catch (error) {
                    Log.log().error(
                        "Vault: Error while trying to renew token: " + error + ". Trying to renew in 30 seconds.",
                    );
    ...
    private async errorTransformer<T>(where: string, fn: () => Promise<T>): Promise<T> {
        try {
            ...
            if (error.response || error.request) {
                // do not log full responses or requests for security reason
                ...
                const msg =
                    ...
                    error.message +
                    ", error descriptors: " +
                    JSON.stringify(errorDescriptors);
