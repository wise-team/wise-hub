<!-- src/components/views/rulesets/rules/TagsRuleComponent.vue -->
<template>
    <div class="tags-rule-component">
        <b-input-group prepend="Mode">
            <b-input-group-prepend>
                <b-form-select v-model="mode" :options="modes" :disabled="!enabled" class="tags-mode-select" />
            </b-input-group-prepend>
            <b-form-input type="text" :disabled="!enabled" v-model="tags" class="t-rule-tags-list-input"></b-form-input>
        </b-input-group>
...
import { TagsRule } from "steem-wise-core";
...
export default Vue.extend({
    ...
    computed: {
        ...
        tags: {
            get(): string { return this.rule.tags.join(", "); },
            set(tagsStr: string): void {
                const tags: string [] = tagsStr.split(",").map(e => e.trim());
                this.saveRule(_.set(_.cloneDeep(this.rule), "tags", tags));
            }
        },
        mode: {
